<template>
    <div id="app" class="container">
      ...
      <div class="mb-3">
        ...
        <input v-model.number="year" type="number" id="year" class="form-control" />
      </div>
      <div class="mb-3">
      ...
      <button @click="generateCalendar" class="btn btn-primary mb-3">달력 생성</button>
        ...
        <div class="calendar mb-4">
          ...
          >
            ...
            <div v-if="day" class="transactions">
              <div v-for="transaction in transactionsByDate[day]" :key="transaction.id" class="transaction">
      ...
      <div v-if="selectedDate">
        ...
        <ul class="list-group mb-3">
          <li v-for="(transaction, index) in transactionsByDate[selectedDate]" :key="index" class="list-group-item">
            {{ transaction.type === 'income' ? '수입' : '지출' }}: {{ transaction.amount }}
            <button @click="editTransaction(index)" class="btn btn-sm btn-secondary float-end ms-2">편집</button>
            <button @click="deleteTransaction(index)" class="btn btn-sm btn-danger float-end">삭제</button>
          </li>
        </ul>
      </div>
      <div class="mb-3">
      ...
      <button @click="addTransaction" class="btn btn-success mb-3">추가</button>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters(['transactionsByDate', 'totalIncome', 'totalExpense', 'balance']),
    },
    methods: {
